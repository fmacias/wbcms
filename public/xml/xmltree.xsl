...
<![CDATA[
    ...
    <xsl:template match="node()">
        ...
        <ul class="wbcmsXMLNode">
            <xsl:if test="$content=''">
                <a style="font-weight:bold;font-size:110%" onclick = "expandElement(this)">-</a>
            </xsl:if>
            ...
            <xsl:for-each select="namespace::node()">
                ...
                <xsl:if test='$currentURI != "http://www.w3.org/XML/1998/namespace"'>
                    <xsl:variable name="parentNameSpace">
                        <xsl:for-each select="$parentChildNodes">
                            <xsl:if test='name() = $currentQName'>exist</xsl:if>
                        </xsl:for-each>
                    </xsl:variable>
