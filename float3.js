...
************************************************************************************************************/
	...
	var oDIVModal = {
			...
			//	var bodyHeight = BodyElement.lastChild.offsetTop+BodyElement.lastChild.offsetHeight;
				//alert(window.innerHeight);
				var layer = createHTMLElement("div",null,null,"IdModalLayer");
				layer.style.position = "absolute";
				layer.style.top = "0px";
				layer.style.left = "0px";
				layer.style.width = getBodyWidth()+"px";
				layer.style.height = getBodyHeight()+"px";
				layer.style.background = "white";
				layer.style.zIndex=currentZIndex;
	...
	*/	
	...
	function hideWindow()
	{
		...
		oDIVModal.unset();//fer
		ignoreMove =false;
		document.getElementById('dhtml_goodies_id' + numericId).afterClose();
	}
	...
	function initWindows(e,divObj)
	{
		...
		for(var no=0;no<divs.length;no++){
			...
			if(divs[no].className=='dhtmlgoodies_window'){	
				...
				var oSelfWindow =divs[no];
				oSelfWindow.afterClose = function(){};//fernando: Polimorphic function called after hideWindow();
			...
			if(divs[no].className=='dhtmlgoodies_window_footer')
			{
				...
				buttonsDiv.className = 'dhtmlgoodies_buttons';
				buttonsDiv.style.zIndex="999";
				...
				divs[no].appendChild(buttonsDiv);
			}
		}	
		if(!divObj)
		{
	...
	function createNewWindow(width,height,left,top,bIsSelectForm,sIFrameSRC)
	{
		...
		{
			var IFrame = document.createElement("iframe");
			IFrame.setAttribute("id", "dhtmlgoodies_IFrame");
			IFrame.setAttribute("name", "dhtmlgoodies_IFrame");
	...
	function customFunctionCreateWindow(width,height,left,top,sHTML,bModal,bIsSelectForm,sIFrameSRC)
	{		
		...
		if ((bIsSelectForm==true)&&(sIFrameSRC))
		{
			alert("The window must contain either a reference of another page or may be a selection window!");
			return false;
		}
		left = (left==undefined)?0:left; 
		top = (top==undefined)?0:top;
		var divId = createNewWindow(width,height,left,top,bIsSelectForm,sIFrameSRC);
	...
	function doAction()
	{
		...
		var numericId = dhtml_goodies_id.id.replace(/[^0-9]/g,'');
		var windowContent = document.getElementById('windowContent' + numericId);
		dhtml_goodies_id.childNodes[0].childNodes[2].childNodes[1].onclick();//hidewindow
		windowContent.doAction();
	}
