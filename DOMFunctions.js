...
function CreateDOMElements(Node_in,NodeToAdd,DOMDocument)
{
	try
	{
		...
		if (Node_in.nodeType)
		{
			...
			{
			//	alert("add ATtribute:"+Node_in.nodeName);
	...
	try
	{
		if (Node_in.nodeType)
		{
			...
			if (Node_in.nodeType==1)//is one Element
			{
				...
				if (bInnerContent)
				{
					if (currentInnerString)sInnerNode = "\n"+currentInnerString+"\n";
...
function IsSGML(nodeName)
{
	...
	try
	{
		/*var parentNode = Node_In.parentNode;
		if (parentNode)
		{
			if (parentNode.nodeName.toUpperCase() =="HEAD")html,head
			{*/
...
function createHTMLElement(sNodeNameIn,sNodeText,sClassNameIn,sElementId,sInnerHTMLIn,sStyleIn,sAttributesIn,sNodeValue)
{
	try
	{
	 if (sNodeNameIn)
	 {
		...
		if (sStyleIn)
		{
			//currentHTMLElement.setAttribute("style",sStyleIn);
			...
			for (var x=0;x<aStyle.length;x++)
			{
				...
				{
					currentHTMLElement.style[aCurrentProperty[0]]=aCurrentProperty[1];
				}
			}*/
			var aStyle = sStyleIn.split(";");
			for (var x=0;x<aStyle.length;x++)
			{
				...
				if (aCurrentProperty[0])
				{
					...
					if (matched)
					{
						...
						{
							//alert(StylePropName+" "+matched[y]);
							//alert(StylePropName+ "->length: "+matched[y].length+" "+matched[y][0]+" "+ matched[y][1]+" "+matched[y]);
		...
		if (sAttributesIn)
		{
			...
			//var sCurrentStyle = document.createAttribute("style");
			//sCurrentStyle.value = sElementId;
			//AppendChild(currentHTMLElement,sCurrentStyle);
		}
		if (sNodeValue)
		{
...
function getHTMLElementWidth(thisNode,Width)
{
	try
	{
		...
		if (thisNode.nodeType==1)
		{
			...
			{
				for (var x=0;x<thisNode.childNodes.length;x++)
				{
					if(thisNode.childNodes[x].nodeType==1)
					{
						if (thisNode.childNodes[x].offsetWidth>Width)
						{
							Width = thisNode.childNodes[x].offsetWidth;
							getHTMLElementWidth(thisNode.childNodes[x],Width);
						}
					}
				}
			}*/
		...
		if (thisNode.nodeType==1)
		{
			...
			/*if (thisNode.childNodes.length>0)
			{
				for (var x=0;x<thisNode.childNodes.length;x++)
				{
					if(thisNode.childNodes[x].nodeType==1)
					{
						Height = getHTMLElementHeight(thisNode.childNodes[x],thisNode.childNodes[x].offsetHeight);
					}
				}
			}*/
		}
...
function onDOMLoaded()//rename to on windowLoaded: at this point of view its posible to access to the script element belong to the window object but not to the element related with the body.
{
	document.DOMLoaded();
}
