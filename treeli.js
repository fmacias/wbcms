var CurrentNode;//XMLNode is currently selected
...
function setSelectedDiv(currentElement)
{
	try
	{
		...
		alert("setSelectedDiv fails: "+e.message);
	}
}*/
...
{ //Currently selected node representation in the Tree
	...
	getRelatedNodeName:function(){
		...
		{
			if (this.element){
				if (this.element.RelatedNodeName) return this.element.RelatedNodeName;
			}
			return "";
		}catch(e){
...
var CurrentPreviewElement={ //Currently selected node representation in the IFrame 
		...
		removeHightLight:function()
		{
			try
			{
				if (this.highlightElement)
				{
					if (this.highlightElement.parentNode) this.highlightElement.parentNode.removeChild(this.highlightElement);
					this.highlightElement = null;
				}
			}catch(e){
...
function editNode__()
{
	...
	{
		if (XMLNewChild) XMLNewChild = XMLNewChild.childNodes[ArrayWithChilds[x]];
	}
	if (XMLNewChild)
	{
	...
	/*
		...
		*/
		...
		}else
		{
			...
			alert(curHTMLElement.innerHTML);
			*/
			var innerHTML = XMLNewChild.innerHTML;
		}
		//alert("element value: "+innerHTML);
		if (innerHTML)
		{
			var regGetHightLight = /<DIV.+id="Id_hightlight".*><\/DIV>/;
...
function hightlightNode()
{
	try
	{
		...
		for(var x=0;x<ArrayWithChilds.length;x++)
		{
			if (thisNode) thisNode = thisNode.childNodes[ArrayWithChilds[x]];
		}
		if (thisNode)
		{	
...
function createTreeNode(Node_in,ChildNodeIndex)
{
	try
	{
		...
		var imgExpand = createHTMLElement("img",null,"BTNDownUp",null,null,null,"alt=Down_Up,src=up.png,alt=up/down");
		...
		imgExpand.onclick = function ()
		{
			try
			{
				...
				if (pngFileName=="up.png")
				{
					...
					this.setAttribute("src","down.png");
				}else{
					parentUl.lastChild.style.display = "none";
					this.setAttribute("src","up.png");
				}
			}catch(e){
				alert("createTreeNode.imgExpand.onclick fails: Error: "+e.message);
			}
		};
		...
		AnclaElement.onclick = function()
		{
			try
			{
				...
				if (thisUl.nodeIndexList) ArrayWithChilds = thisUl.nodeIndexList.split(",");
			...
			{
				alert("createTreeNode.AnclaElement.onclick fails: Error: "+e.message);
			}
		};
...
/*function changeCurrentDom___()
{
	...
	if ((fileExtension=="HTML")||(fileExtension=="HTM")||(fileExtension=="XHTML"))
	{
		...
		}else if (bIsChildHead)
		{
			...
			alert(HTMLTempElement.innerHTML);
			...
			alert("new Child = "+newChild.innerHTML);
		...
		if ((selectedFile.ParserMimeType=="application/xml") || ((extension == "XML")&&(selectedFile.contentType != "text/html")))
			{
				...
				for(var x=0;x<ArrayCurrentNodeRef.length;x++)
				{
					if (selectedNode) selectedNode = selectedNode.childNodes[ArrayCurrentNodeRef[x]];
				}
...
/*function editAtt(sRelatedNodeAttributes)
{
	try
	{
		//alert(currentEditor.NodeArea.oElement);
		//alert(currentEditor.NodeArea.NodeAttributes.oElement.childNodes.length+".>from DOM"+document.getElementById("NodeAtributes").childNodes.length);
	...
	try
	{
		...
		dialog.doAction= function()
		{
			try
			{
				...
				alert("treeli.js::addAttribute()::dialog.windowContent.doAction fails. Error:"+e.message);
			}
		};
	}catch(e){
