...
function dragSeparatorElement()
{
	...
	this.setTop=function(eClientY,maxHeight)
	{
		...
		/*var mainElementBorder = 2;
		//var realHight = eClientY-this.separator.height*2;
		var mainHight = eClientY-nTop-WINDOW_CONTAINER_BORDER*2;
		var winBodyHeight = this.separator.mainElement.childNodes[4].offsetHeight;
		var winBodyTop = this.separator.mainElement.childNodes[4].offsetTop;
		var rest = mainHight-winBodyHeight-winBodyTop-WINDOW_CONTAINER_BORDER;
		//
		var mainContainerHeight = winBodyHeight+rest;
		
		
		//
		//this.separator.mainElement.childNodes[4].style.height = (mainContainerHeight-WINDOW_CONTAINER_BORDER)+"px";
		//
		this.separator.mainElement.style.height = (mainHight)+'px';*/
		for (var y=0;y<this.separator.relElements.length;y++)
		{
			//
...
function genericContainer()
{
	...
	{
		/*this.width = getHTMLElementWidth(this.oElement);
		this.height = getHTMLElementHeight(this.oElement);
		this.top = getHTMLElementTop(this.oElement.offsetTop);
		this.left = getHTMLElementLeft(this.oElement);*/
...
function Separator(top,left,width,height,orientation)
{
	...
	//this.width =2;
	//this.size =WINDOW_CONTAINER_BORDER;
	this.size =4;
	//this.border =0;separator.border
	//this.NodeValue=".";
	//Z_INDEX++;
	...
	//this.Style="position:absolute;top:"+top+"px;left:"+left+"px;color:red;cursor:move";
...
/*reload.enable = function()
{
};
reload.disable = function(){
...
/*changeDOM.enable = function(){
	
};
changeDOM.disable = function(){
...
/*createAttribute.enable = function(){
	
};
createAttribute.disable = function(){
...
/*saveDOM.enable = function(){
	
};
saveDOM.disable = function(){
...
/*removeElement.enable = function(){
	
};
removeElement.disable = function(){
...
/*appendChild.enable = function(){
	
};
appendChild.disable = function(){
...
/*insertBefore.enable = function(){
	
};
insertBefore.disable = function(){
...
avalMetas.onclick = function()
{
	try
	{
		...
		alert("avalMetas button fails. error: "+e.message);
	}
};
...
/*renameElement.enable = function(){
	
};
renameElement.disable = function(){
...
{
	alert("headerElement fails: "+e.message);
}
}
...
function winTitleElement()
{
	...
	this.oElement.onmouseover = function()
	{
		try
		{
			...
			alert("winTitleElement: this.oElement.onmouseover"+e.message);
		}
	};
	/*this.oElement.onmouseout = function()
	{
		try
		{
			if (this.HighLight)
			{
				this.HighLight = this.removeChild(this.HighLight);
				this.HighLight = null;
			}
		}catch(e){
			alert("winTitleElement: this.oElement.onmouseout"+e.message);
		}
	};*/
	}catch(e){
		alert("winTitleElement: "+e.message);
	}
}
winTitleElement.prototype = new oHTMLElement();
/*winTitleElement.prototype.onOver = function()
{
	try
	{
		var absoluteTop = getHTMLElementTop(this.oElement);
		var absoluteLeft = getHTMLElementLeft(this.oElement);
		var elementHeight = this.oElement.offsetHeight+this.Border*2;
	//	alert("position:absolute;top:"+absoluteTop+"px;margin:0;padding:0;left:"+absoluteLeft+"px;background:#FFFFCC;height:"+elementHeight+"px");
		this.HighLight = createHTMLElement("div",oSelf.titleDescription,null,null,null,"position:absolute;top:"+absoluteTop+"px;margin:0;padding:0;left:"+absoluteLeft+"px;background:#FFFFCC;height:"+elementHeight+"px",null);
		this.HighLight = document.body.appendChild(this.HighLight);
		var oSelfHTMLElement = this.oElement;
		this.HighLight.onmouseout = function()
		{
			oSelfHTMLElement.parentNode.removeChild(this);
			oSelf.HighLight = null;
		};
	}catch(e){
		alert("winTitleElement: this.oElement.onmouseover"+e.message);
...
containerWindow.prototype.draw =function()
{
	...
	{
		alert("containerWindow fails: "+e.message);
	}
};
//
function containersOperations(){}
	...
	containersOperations.prototype.enableButtons = function()
	{
		...
		{
			alert("containersOperations.enableButtons fails: "+e.message);
		}
	};
	containersOperations.prototype.disableButtons = function()
	{
		...
		{
			alert("containersOperations.disableButtons fails: "+e.message);
		}
	};
	containersOperations.prototype.closeWindow = function()
	{
		...
		{
			alert("containersOperations.closeWindow fails: "+e.message);
		}
	};
...
function genericTreeContainer(top,left,width,height)
{
	try
	{
		...
		//this.container.header.oElement.innerHTML ="tree";
	...
	try
	{
		...
		this.draw = function()
		{
			...
			this.TextArea.oElement = this.container.winBody.oElement.appendChild(this.TextArea.oElement);
				...
				alert("genericSourceCodeContainer"+e.message);
			}
		};
		this.adaptBody = function()
		{
	...
	try
	{
		...
		this.draw = function()
		{
			try
			{
				...
				alert("genericPreviewContainer: "+e.message);
			}
		};
		this.adaptBody = function()
		{
		...
		this.addIFrame = function(sIFrameSRC)
		{
			...
			//IFrame.width =WidthPercent;
			//IFrame.height =heightPercent;
			IFrame.width ="100%";
			IFrame.height ="100%";

			/*IFrame.setAttribute("src", sIFrameSRC);
			IFrame.setAttribute("frameBorder", "0");
			IFrame.setAttribute("scrolling", "false");
			IFrame.setAttribute("margin", "0 0 0 0");
			IFrame.setAttribute("padding", "0 0 0 0");
			IFrame.setAttribute("style","width:100%;height:"+heightPercent);*/
			var appededIFrame = this.container.winBody.oElement.appendChild(IFrame);
			}catch(e)
			{
				alert("add IFrame fails: "+e.message);
			}
		};
		...
		this.setXMLDisableView = function()
		{
			...
			this.reloadIFrame=function()
			{
				...
				CurrentPreviewElement.enabled=true;
					...
					alert("reloadIFrame() fails Error: "+e.message);
				}
				...
				AJAXTempTreeFile.reply.onReply=function()
				{
					try
					{
						...
						CurrentPreviewElement.enabled=true;
							...
							{
								throw new Error("\nsetXMLDisableView.AJAXTempTreeFile.reply.onReply fails. Error "+e.message);
							}
				};
...
function AttContainer(AttName, AttValue)
{
	...
	{
		this.HTMLElementName = "div";
		//this.Style = "border:1px solid #CCCCCC;margin: 0.5%;padding:0px 0px 5px 0px;width:96%";
	...
	{
		alert("class AttContainer fails: "+ e.message);
	}
};
...
function NodeEditor()
{
	...
	this.addAttribute = function(AttName, AttValue)
	{
		...
		{
			alert("class NodeEditor.addAttribute fails: "+ e.message);
		}
	};
	this.removeAttribute = function(AttName)
	{
		...
		{
			alert("class NodeEditor.removeAttribute fails: "+ e.message);
		}
	};
	this.disableRemoveButtons = function()
	{
		try
		{
			...
			alert("class NodeEditor.disableRemoveButtons fails: "+ e.message);
		}
	};
	...
	this.enableRemoveButtons = function()
	{
		try
		{
			...
			alert("class NodeEditor.enableRemoveButtons fails: "+ e.message);
		}
	};
	...
	try
	{
		...
		this.draw = function()
		{
			try
			{
				...
				alert("genericEditorContainer: "+e.message);
			}
		};
		this.adaptBody = function()
		{
		...
		this.editNode = function(NodeValue,sNodeAttributes,NodeInnerHTML)
		{
			...
			if (aRelatedNodeAttributes.length>0)
			{
				try
				{
					...
					for (var x=0;x<aRelatedNodeAttributes.length;x++)
					{
						if (aRelatedNodeAttributes[x])
						{
							var AttName = "";
							var AttContent = "";
							//var aATT = aRelatedNodeAttributes[x].split("=");
						//	var re = /=/;
							//match(re);
							//var re = /^(\S+)=(.*)$/;
							var re = /^(\S+)=([\S\s\W.]*)/;
				...
				{
					alert("class genericEditorContainer.editNode fails: "+ e.message);
				}
			}
		};
	}catch(e){
...
function mainContainer(nwidth,nheight)
{
	...
	this.draw = function()
	{
		try
		{
			...
			alert("mainContainer.draw()fails: "+e.message);
		}
	};
	...
	this.setTreeView = function()
	{
		try
		{
			...
			//alert("separator background: "+this.tree.separator.oElement.style.background);
			//
			...
			alert("mainContainer.setTreeView()fails: "+e.message);
		}
	};
...
function windowFooter()
{
	try
	{
		...
		alert("Error:Class windowFooter: "+e.message);
	}
}
...
function floatWindow(windowId,top,left,width,height)
{
	try
	{
		...
		//
		//this.container.header.closeButton  = new oHTMLElement();
		
		/*var closeButton = this.container.header.closeButton;
		closeButton.HTMLElementName = "span";
		closeButton.ClassName = "maxButton";
		closeButton.Style = "float:right";
		closeButton.InnerHTML = "<img src='images/close.gif' alt='Maximize' border=''/>";
		closeButton.set();*/
		//
		//
		//this.container.footer = null;
		//alert("float window done");
		this.draw = function(sHTML, sIFrameSRC,bModal,bUserInputRequired)
		{
			...
			//a float window has also a close button. This item must be done in the next version
			/*var minButton = this.container.header.minButton.oElement;
			var maxButton = this.container.header.maxButton.oElement;
			minButton.parentNode.removeChild(minButton);
			maxButton.parentNode.removeChild(maxButton);*/
			//removeChilds(this.container.header.oElement.firstChild);
			//closeButton.oElement = this.container.header.oElement.firstChild.appendChild(closeButton.oElement);
			...
			this.container.header.oElement.onmousedown = function(e)
			{
				...
				alert("Error:Class floatWindow.draw() fails: "+e.message);
			}
		};
		this.adaptBody = function()
		{
		...
		this.onclosing = function()
		{
			try
			{
				...
				this.closeWindow();
				/*var removedWindow = this.container.oElement.parentNode.removeChild(this.container.oElement);
				var modalLayer = document.getElementById("IdModalLayer");
				if (modalLayer)
				{
					var removedLayer = modalLayer.parentNode.removeChild(modalLayer);
				}*/
				//this.container.afterClose();
			}catch(e){
				alert(e.message);
			}
		};
		...
		alert("Error:Class floatWindow::constructor()"+e.message);
	}
}
...
/*function floatWindow2(top,left,width,height)
{
	try
	{
		//container
		this.container =  new containerWindow();
		this.Border = WINDOW_CONTAINER_BORDER;
		this.container.ClassName = "flotaWindow";
		this.container.Style = "position:absolute;top:"+top+"px;left:"+left+"px;width:"+width+"px;height:"+height+"px;border:"+this.Border+"px solid #707070;overflow:auto;background-color:#CCCCCC";
		//this.container.header.oElement.innerHTML ="tree";
		//this.container.constructor();
		this.draw = function()
...
floatWindow2.prototype.container = new Object();
floatWindow2.prototype = new containersOperations();*/
...
//BrowserDetect thanks to http://www.quirksmode.org
var BrowserDetect = {
		init: function () {
...
if (BrowserDetect.browser == "Explorer")
{
	...
	{
		var alert = "Yo need at least Microsoft Internet Explorer 8";
		alerts.push(alert);
	}
	if (BrowserDetect.version>=8)
	{
...
function load()
{
		try
		{
			...
			//
			//
			//reload.setrelMenuOption(oMenuOptionIn);
			changeDOM.setrelMenuOption(oChange);
			...
			oEditorContainer = oMainContainer.editor; 
			oSourceCodeEditor = oMainContainer.sourceCodeEditor;
			/*var float1 =new floatWindow("1",20,20,220,120);
			var float2 = new floatWindow("",30,30,230,130);
			float1.draw();
			float2.draw();*/
			oViewLoadProject.createWindow();
			oChangeProjekt.enable();
			//
			//
			//oEditorContainer.editNode('nodeName','test1=3#;#test2=2#;#test3=3#;#','<p>hola</p>');
			//oPreviewContainer.addIFrame("http://www.gara.net");
			//var floatTest = new floatWindow("windowId");
			//floatTest.draw(null,"http://www.gara.net",true,true);
	...
	{
		alert("document.DOMLoaded: "+error.message);
	}
}
