...
/*function oDivsToLoad(sJSONStr)
{
	...
	this.SetHTMLNodeInThisSection = function (aSectionIds_in,CurrentSection,currentDiv)//append each Node found in each related section in the corresponding HTMLDIVElement 
	{
		...
		if (aSectionIds_in[LasSectionIdIndex].SectionsIds)
		{
			...
			{
				//Si hay seciones pertenecientes a otras secciones, el contenido de estas secciones se agregarán en 
		...
		{ 
			...
			appendedLayer.firstChild.onclick= function()
			{
				try
				{
					...
					alert("wcms.js::getDisableLayer::appendedLayer.onmousseover fails:"+e.message);
				}
			};
			...
			close.onclick=function()
			{
				try
				{
					...
					alert("wcms.js::getDisableLayer::close.onclick fails:"+e.message);
				}
			};
		}
		if (appendedLayer==null){ throw new Error("Disable Layer was not created!");}
		return appendedLayer;
	}catch(e){
		alert("wcms.js::getDisableLayer fails:"+e.message);
	}
}
function showErrorDisableLayer(disableLayer,errorString)
{
	try
	{
		...
		{
			alert("There is no Error seccion defined in the layer");
			return;
		}	
		if (!(disableLayer.childNodes[2].className="disable_error_message"))
		{
			alert("There is not Error seccion defined in the layer");
			return;
		}
		...
		alert("wcms.js::showErrorDisableLayer fails:"+e.message);
	}
}
function showSuccessDisableLayer(disableLayer)
{
	try
	{
		...
		alert("wcms.js::showSuccessDisableLayer fails:"+e.message);
	}
}