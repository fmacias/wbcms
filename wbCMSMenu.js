...
Menu.prototype.displayMain = function( )//return true or false and alert
{
	try
	{
		...
		var oHTMLDivMainMenu = createHTMLElement("div","","menuBar",this.menuIndex,null,"position:relative;top:0px;left:0px;display:block;padding:0;width:100%",null);//;width:"+window.getBodyWidth()+"px" width:"+window.getBodyWidth()+"px"
		...
		for (indexSubmenu in this.menuItems)
		{
			...
			oHTMLDivMenuOption.onmouseover = changeBackground;
...
Menu.prototype.displayMainMenuOption = function(self,top,left)//try catch return throw new
{
	try
	{
		...
		if (document.getElementById("_"+this.menuIndex))
		{
			...
			{
				//alert(getHTMLElementHeight(oHTMLOptionContainer));
				var currentTop = top;
				//"clear:both;position:absolute;top:"currentTop+"px;left:"+left+";padding:5px 0 0 0;cursor:pointer;cursor:hand;overflow:visible"
				var oHTMLDivOption = createHTMLElement("div","","menuOption",this.menuItems[oIndex].menuIndex,null,"clear:both;display:block;padding:0 0 0 0;cursor:pointer;cursor:hand;overflow:visible",null);
				...
				addedOption.onmouseover = changeBackground;
				addedOption.onmouseout = delBackground;
				//
				...
				if (this.menuItems[oIndex].numChilds>0)
				{
					...
					addedOption.onclick = showSubmenu;
					addedOption.setAttribute("relContainer","_"+this.menuIndex);
				}else{
					addedOption.onclick = doMenuAction;
				}
...
function openMenu()//return nothing, on Error Alert
{
	try
	{
		...
		alert("wbCMSMenu.js::openMenu() fails. Error: "+e.message);
	}
}
function showSubmenu()//return nothing, on Error Alert
{
	...
	{
		alert("wbCMSMenu.js::showSubmenu() fails. Error: "+e.message);
	}
}
function doMenuAction()//return nothing, on Error alert
{
	...
	{
		alert("wbCMSMenu.js::doMenuAction() fails. Error: "+e.message);
	}
}
function changeBackground()//return nothing, on Error Alert
{
	...
	{
		alert("wbCMSMenu.js::changeBackground() fails. Error: "+e.message);
	}
}
function delBackground()//return nothing, on Error Alert
{
	...
	{
		alert("wbCMSMenu.js::delBackground() fails. Error: "+e.message);
	}
}
function checkDelBackground()//return nothing, on Error Alert
{
	...
	{
		alert("wbCMSMenu.js::checkDelBackground() fails. Error: "+e.message);
	}
}